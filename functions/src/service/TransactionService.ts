...
export const getOneMonthsDates = (date: string, frequency: string) => {
	let numIter;
	const datesArray = [date];
	if (frequency === 'Monthly' || frequency === 'Sporadic' || frequency === 'Once') return [date];
	if (frequency === 'Weekly') numIter = 4;
	else numIter = 30;
	for (let i = 1; i < numIter; i++) {
		datesArray.push(incrementDate(date, frequency, i));
	}
	return datesArray;
};
export const getDatesForPeriod = (date: string, frequency: string, amount: number = 6) => {

};

export const formatInitialTransactionData = (transaction: Transaction) => {
	console.log('formatting initial transaction response for', transaction.title);
	const data = {
		transaction,
		dates: [transaction.nextDueDate]
	};
	if (!transaction.isRecurring()) return data;
	return data;
};
